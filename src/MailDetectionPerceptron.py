...
def classify(matrix, rows, featureweightsList):
    outputList = []
    for j in range(0, rows):
        count = 0
        sumweight = 0.00
        for weight in featureweightsList:
            sumweight += float(weight) * matrix[j][count]
            count += 1
        if(sumweight > 0):             
            outputList.append(1)
        else:            
...
def trainPerceptron(dataframe, featureweightsList):
    ...
    for k in range(0,10):
        outputList = classify(matrix, rows, featureweightsList)
        for j in range(0, rows):
            update = learningrate*(matrix[j][columns-1] - outputList[j])
            for i in range(0, columns-1):
                featureweightsList[i] = featureweightsList[i] +  update* matrix[j][i]
                
    return featureweightsList


def testPerceptron(testdataframe, featureweightsList, isStopWordUsed):
    matrix = testdataframe.values
    totalrow = testdataframe.shape[0]
    totalcolumn = testdataframe.shape[1]
    correctCount = 0
    outputlist = classify(matrix, totalrow, featureweightsList)
...
def main(stopwordsList, traindirectorypoststemming, testdirectorypoststemming, isStopWordUsed):
    ...
    featureweightsList = trainPerceptron(dataframe, featureweightsList)
    ...
    testPerceptron(testdataframe, featureweightsList, isStopWordUsed)   
